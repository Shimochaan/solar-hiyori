...
## マイルストーン:Web版の機能要件フリーズ(2026-06-12)
ここまででWeb版(MVP)の**機能要件は一旦完了=フリーズ**。公開済み:
- アプリ本番: https://solar-hiyori.vercel.app
- ソース: https://github.com/Shimochaan/solar-hiyori (Public)

**デザインの磨き込み(地図/3Dのイラスト性・UIの動き)はモバイルアプリ化フェーズへ繰り越し**。
理由: Expo化では地図(→react-native-maps系)・3D(→react-three-fiber等)をネイティブ向けに
作り直すため、今Web版を作り込むと捨て工数になる。よってWeb版は「フラット淡色3D+CARTO地図」で十分とする。

次フェーズの選択肢(オーナー判断待ち):
- (A) モバイルアプリ化(Expo)+動きのあるUI設計 ← デザイン性はここで一気に上げる
- (B) 完全イラスト低ポリ3Dのプロトタイプ(Three.js / 積み木的な近隣建物表現)
- (C) 需要検証(LP+広告)に回して「そもそも刺さるか」を先に見る
